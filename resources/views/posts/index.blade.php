  ...
  <div class="grid lg:grid-cols-2 lg:gap-y-16 gap-10">
    ...
    <a class="group block rounded-xl overflow-hidden focus:outline-none" href="#">
      <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5">
        ...
        <button class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition duration-300 flex items-center gap-2">
       
  <a href="{{ route('posts.create', $posts) }}">Create New Post</a>
</button>
        <div class="grow">
        @foreach($posts as $post)
        <!--image-->
        <img src="{{ $post->post_image }}" alt="Plus Icon" class="w-20 h-20">
        <!--image-->
          <h3 class="text-xl font-semibold text-gray-800 group-hover:text-gray-600 dark:text-neutral-300 dark:group-hover:text-white">
            {{$post->post_title}}
            {{$post->tags->pluck('tag_name')->implode('')}}
          </h3>
